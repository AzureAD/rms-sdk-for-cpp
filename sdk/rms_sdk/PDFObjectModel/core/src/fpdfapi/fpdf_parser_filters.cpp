...
void CFX_DataFilter::FilterIn(FX_LPCBYTE src_buf, size_t src_size, CFX_BinaryBuf& dest_buf)
{
    ...
    m_SrcPos += static_cast<FX_INT32>(src_size);
    if (m_pDestFilter) {
...
{
  FX_UNREFERENCED_PARAMETER(width);
  FX_UNREFERENCED_PARAMETER(height);
...
size_t CPDF_StreamFilter::ReadBlock(FX_LPBYTE buffer, size_t buf_size)
{
    if (m_pFilter == NULL) {
        ...
        if (read_size > buf_size) {
            read_size = static_cast<FX_DWORD>(buf_size);
        }
    ...
    if (m_pBuffer) {
        read_size = static_cast<FX_DWORD>(ReadLeftOver(buffer, buf_size));
        if (read_size == buf_size) {
            return read_size;
        }
        buffer += read_size;
    ...
    while (bCondition) {
        ...
        if (m_pFilter->NeedNewSrc() || m_pFilter->AbortAll()) {
            ...
            if (src_size > static_cast<int>(m_SrcBufferSize)) {
...
size_t CPDF_StreamFilter::ReadLeftOver(FX_LPBYTE buffer, size_t buf_size)
{
    ...
    m_BufOffset += static_cast<FX_INT32>(read_size);
...
void CPDF_DecryptFilter::v_FilterIn(FX_LPCBYTE src_buf, size_t src_size, CFX_BinaryBuf& dest_buf)
{
    ...
    crypto_handler_->DecryptStream(m_pContext, src_buf, static_cast<FX_DWORD>(src_size), dest_buf);
}
...
void CPDF_FlateFilter::v_FilterIn(FX_LPCBYTE src_buf, size_t src_size, CFX_BinaryBuf& dest_buf)
{
    ...
    if (m_bExhaustBuffer) {
        FPDFAPI_FlateInput(m_pContext, src_buf, static_cast<unsigned int>(src_size));
    }
...
void CPDF_PredictorFilter::v_FilterIn(FX_LPCBYTE src_buf, size_t src_size, CFX_BinaryBuf& dest_buf)
{
    ...
    while (bCondition) {
        ...
        if (read_size > src_size) {
            read_size = static_cast<FX_DWORD>(src_size);
        }
                    ...
                    if (byte >= '!' && byte <= 'u') {
                        ...
                        if (m_CharCount == 5) {
                            for (int i4 = 0; i4 < 4; i4 ++) {
                                dest_buf.AppendByte((FX_BYTE)(m_CurDWord >> (3 - i4) * 8));
                            }
                            m_State = 0;
                        }
                    } else if (byte == '~') {
                        if (m_CharCount > 1) {
                            for (int i5 = m_CharCount; i5 < 5; i5 ++) {
                                m_CurDWord = m_CurDWord * 85 + 84;
                            }
                            for (int i6 = 0; i6 < m_CharCount - 1; i6 ++) {
                                dest_buf.AppendByte((FX_BYTE)(m_CurDWord >> (3 - i6) * 8));
                            }
                        }
                        m_State = 2;
                    }
...
void CPDF_AsciiHexFilter::v_FilterIn(FX_LPCBYTE src_buf, size_t src_size, CFX_BinaryBuf& dest_buf)
{
    for (FX_DWORD i = 0; i < src_size; i ++) {
        ...
        if (byte >= '0' && byte <= '9') {
            ...
            if (m_State) {
                dest_buf.AppendByte(static_cast<FX_BYTE>(m_FirstDigit * 16));
            }
            ReportEOF(static_cast<FX_DWORD>(src_size - i - 1));
            return;
        }
        if (m_State == 0) {
            ...
            dest_buf.AppendByte(static_cast<FX_BYTE>(m_FirstDigit * 16 + digit));
            m_State --;
        }
    }
}
        ...
        switch (m_State) {
            case 0: {
                    if (byte < 128) {
                        ...
                        ReportEOF(static_cast<FX_DWORD>(src_size - m_SrcOffset - 1));
                        return;
                    } else {
                        m_State = 2;
                        m_Count = 257 - byte;
