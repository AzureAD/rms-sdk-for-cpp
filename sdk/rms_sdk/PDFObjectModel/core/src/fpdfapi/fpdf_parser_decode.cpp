...
FX_BOOL PDF_DataDecode(FX_LPCBYTE src_buf, size_t src_size, const CPDF_Dictionary* pDict,
...
CFX_WideString PDF_DecodeText(FX_LPCBYTE src_data, FX_DWORD src_len, CFX_CharMap* pCharMap, const CFX_ByteString& endcoding)
{
    ...
    if (src_len >= 2 && ((src_data[0] == 0xfe && src_data[1] == 0xff) || (src_data[0] == 0xff && src_data[1] == 0xfe))) {
        ...
        int dest_pos = 0;
        FX_DWORD surrogateUnicodeFirst = 0;
    ...
    } else if (src_len >= 3 && (src_data[0] == 0xef && src_data[1] == 0xbb && src_data[2] == 0xbf)) {
        if (src_len == 3) {
            return result;
        }
        result = CFX_WideString::FromUTF8((const char*)(src_data + 3), src_len - 3);
        ...
        if(endcoding == "utf-8") {
            result = CFX_WideString::FromUTF8((const char*)src_data, src_len);
        } else if(endcoding == "utf-16") {
            result = CFX_WideString::FromUTF16BE((const unsigned short*)src_data, src_len / 2);
        } else {
            FX_LPWSTR dest_buf = result.GetBuffer(src_len);
            for (FX_DWORD i = 0; i < src_len; i ++) {
                dest_buf[i] = PDFDocEncoding[src_data[i]];
...
CFX_WideString PDF_DecodeText(const CFX_ByteString& bstr, CFX_CharMap* pCharMap, const CFX_ByteString& endcoding)
{
    return PDF_DecodeText((FX_LPCBYTE)(FX_LPCSTR)bstr, bstr.GetLength(), pCharMap, endcoding);
}
CFX_ByteString PDF_EncodeText(FX_LPCWSTR pString, int len, CFX_CharMap* pCharMap)
{
    ...
    if (pCharMap == NULL) {
        ...
        result = pCharMap->m_GetByteString(pCharMap, CFX_WideString(pString, len));
        return result;
    }
    if (len > INT_MAX / 2 - 1) {
