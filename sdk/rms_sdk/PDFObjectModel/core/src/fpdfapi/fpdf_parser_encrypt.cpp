...
CPDF_StandardSecurityHandler::CPDF_StandardSecurityHandler()
{
    ...
    m_KeyLen = 0;
    FXSYS_memset32(m_EncryptKey, 0, sizeof(m_EncryptKey));
}
CPDF_StandardSecurityHandler::~CPDF_StandardSecurityHandler()
{
}
...
FX_BOOL CPDF_StandardSecurityHandler::CheckSecurity(CFX_ByteString password, FX_INT32 key_len)
{
...
FX_BOOL CPDF_StandardSecurityHandler::CheckSecurity(FX_INT32 key_len)
{
    ...
    if (CheckSecurity(password, key_len)) {
        return TRUE;
    }
    FX_INT32 count = 0;
    ...
    if (count > 0) {
        return CheckSecurity(password, key_len);
    }
    return FALSE;
}
        ...
        FX_BOOL bOwner, FX_LPBYTE key)
{
    ...
    CPDF_Document* pDocument = m_pParser->GetDocument();
    if (pDocument) {
        CPDF_Object* pRootObj = pDocument->GetIndirectObject(objId);
        if (pRootObj && pRootObj->GetType() == PDFOBJ_DICTIONARY) {
            CPDF_Dictionary* pRoot = static_cast<CPDF_Dictionary*>(pRootObj);
            CPDF_Object* pMetadataObj = pRoot->GetElement(FX_BSTRC("Metadata"));
            if (pMetadataObj) {
        ...
        if (pRootObj) {
            pDocument->ReleaseIndirectObject(objId);
        }
    }
    CPDF_PDFVersionChecker::VersionCheck(m_pParser, FPDF_FEATURE_AES256BitsEncrypt);
    return TRUE;
}
        ...
        FX_BOOL bIgnoreEncryptMeta, FX_LPBYTE key, FX_INT32 key_len)
{
    ...
    FX_BYTE ukeybuf[32];
    FXSYS_memset32(ukeybuf, 0, sizeof(ukeybuf));
    if (m_Revision == 2) {
        ...
        CRYPT_ArcFourCryptBlock(ukeybuf, 16, key, key_len);
        FX_BYTE tmpkey[32];
    ...
    return FXSYS_memcmp32(ukeybuf, (FX_LPVOID)(FX_LPCSTR)ukey, 16) == 0;
}
...
{
    FX_BYTE digest[16];
    FX_Random_GenerateCrypto((FX_LPDWORD)digest, 16 / sizeof(FX_DWORD));
