...
#include "xml_int.h"
const CFX_WideString* CXML_AttrMap::Lookup(const CFX_ByteString& space, const CFX_ByteString& name) const
{
    if (m_pMap == NULL) {
        return NULL;
    }
...
void CXML_AttrMap::SetAt(const CFX_ByteString& space, const CFX_ByteString& name, const CFX_WideString& value)
{
...
void CXML_AttrMap::RemoveAt(const CFX_ByteString& space, const CFX_ByteString& name)
{
    if (m_pMap == NULL) {
        return;
    }
...
static CXML_Element* XML_ContinueParse(CXML_Parser &parser, FX_BOOL bSaveSpaceChars, FX_FILESIZE* pParsedSize, FX_BOOL bSaveSpecialChar)
{
    parser.m_bSaveSpaceChars = bSaveSpaceChars;
    parser.m_bSaveSpecialChar = bSaveSpecialChar;
...
CXML_Element* CXML_Element::Parse(const void* pBuffer, size_t size, FX_BOOL bSaveSpaceChars, FX_FILESIZE* pParsedSize, FX_BOOL bSaveSpecialChar)
{
    CXML_Parser parser;
    if (!parser.Init((FX_LPBYTE)pBuffer, size)) {
        return NULL;
    }
    return XML_ContinueParse(parser, bSaveSpaceChars, pParsedSize, bSaveSpecialChar);
}
CXML_Element* CXML_Element::Parse(IFX_FileRead *pFile, FX_BOOL bSaveSpaceChars, FX_FILESIZE* pParsedSize, FX_BOOL bSaveSpecialChar)
{
    CXML_Parser parser;
...
CXML_Element* CXML_Element::Parse(IFX_BufferRead *pBuffer, FX_BOOL bSaveSpaceChars, FX_FILESIZE* pParsedSize, FX_BOOL bSaveSpecialChar)
{
    CXML_Parser parser;
...
CXML_Element::CXML_Element(const CFX_ByteString& qSpace, const CFX_ByteString& tagName)
    : m_pParent(NULL)
    , m_QSpaceName()
    , m_TagName()
    , m_AttrMap()
...
CXML_Element::CXML_Element(const CFX_ByteString& qTagName)
    : m_pParent(NULL)
    , m_QSpaceName()
    , m_TagName()
    , m_AttrMap()
...
void CXML_Element::GetTagName(CFX_ByteString &tagName, FX_BOOL bQualified) const
{
    if (!bQualified || m_QSpaceName.IsEmpty()) {
        tagName = m_TagName;
        return;
    }
    FX_LPSTR str = tagName.GetBuffer(m_QSpaceName.GetLength() + m_TagName.GetLength() + 2);
    if (!str) {
        return;
    }
    FXSYS_memcpy32(str, (FX_LPCSTR)m_QSpaceName, m_QSpaceName.GetLength());
    str += m_QSpaceName.GetLength();
    *str = ':';
    str ++;
    FXSYS_memcpy32(str, (FX_LPCSTR)m_TagName, m_TagName.GetLength());
...
void CXML_Element::GetNamespace(CFX_ByteString &nameSpace, FX_BOOL bQualified) const
{
    if (bQualified) {
        nameSpace = m_QSpaceName;
        return;
    }
    GetNamespaceURI(m_QSpaceName, nameSpace);
}
CFX_ByteString CXML_Element::GetNamespaceURI(const CFX_ByteString& qName) const
{
...
void CXML_Element::GetNamespaceURI(const CFX_ByteString& qName, CFX_ByteString &uri) const
{
    const CFX_WideString* pwsSpace;
    const CXML_Element *pElement = this;
    do {
...
FX_BOOL CXML_Element::HasAttr(const CFX_ByteString& name) const
{
    CFX_ByteStringC bsSpace, bsName;
    FX_XML_SplitQualifiedName(name, bsSpace, bsName);
    return m_AttrMap.Lookup(bsSpace, bsName) != NULL;
}
FX_BOOL CXML_Element::GetAttrValue(const CFX_ByteString& name, CFX_WideString& attribute) const
{
    CFX_ByteStringC bsSpace, bsName;
    FX_XML_SplitQualifiedName(name, bsSpace, bsName);
...
const CFX_WideString* CXML_Element::GetAttrValuePtr(const CFX_ByteString& name) const
{
    CFX_ByteStringC bsSpace, bsName;
    FX_XML_SplitQualifiedName(name, bsSpace, bsName);
    return m_AttrMap.Lookup(bsSpace, bsName);
}
FX_BOOL CXML_Element::GetAttrValue(const CFX_ByteString& space, const CFX_ByteString& name, CFX_WideString& attribute) const
{
...
const CFX_WideString* CXML_Element::GetAttrValuePtr(const CFX_ByteString& space, const CFX_ByteString& name) const
{
    return m_AttrMap.Lookup(space, name);
}
FX_BOOL CXML_Element::GetAttrInteger(const CFX_ByteString& name, int& attribute) const
{
    CFX_ByteStringC bsSpace, bsName;
    FX_XML_SplitQualifiedName(name, bsSpace, bsName);
...
FX_BOOL	CXML_Element::GetAttrInteger(const CFX_ByteString& space, const CFX_ByteString& name, int& attribute) const
{
...
FX_BOOL CXML_Element::GetAttrFloat(const CFX_ByteString& name, FX_FLOAT& attribute) const
{
    CFX_ByteStringC bsSpace, bsName;
    FX_XML_SplitQualifiedName(name, bsSpace, bsName);
    return GetAttrFloat(bsSpace, bsName, attribute);
}
FX_BOOL CXML_Element::GetAttrFloat(const CFX_ByteString& space, const CFX_ByteString& name, FX_FLOAT& attribute) const
{
    const CFX_WideString* pValue = m_AttrMap.Lookup(space, name);
    if (pValue) {
...
const CFX_WideString* CXML_Element::GetContentPtr(FX_DWORD index) const
{
    index <<= 1;
    if (index >= (FX_DWORD)m_Children.GetSize() ||
            (ChildType)(FX_UINTPTR)m_Children.GetAt(index) != Content) {
...
int CXML_Element::CountElements(const CFX_ByteString& space, const CFX_ByteString& tag) const
{
    int count = 0;
    for (int i = 0; i < m_Children.GetSize(); i += 2) {
        ChildType type = (ChildType)(FX_UINTPTR)m_Children.GetAt(i);
...
CXML_Element* CXML_Element::GetElement(const CFX_ByteString& space, const CFX_ByteString& tag, int index) const
{
    if (index < 0) {
        return NULL;
    }
...
void CXML_Parser::SkipLiterals(const CFX_ByteString& str)
{
    m_nOffset = m_nBufferOffset + (FX_FILESIZE)m_dwIndex;
    if (IsEOF()) {
        return;
