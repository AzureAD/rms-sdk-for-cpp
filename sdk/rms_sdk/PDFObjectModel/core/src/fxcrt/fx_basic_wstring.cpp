...
int CFX_WideString::Compare(FX_LPCWSTR lpsz) const
{
    ...
    int tmp = FXSYS_wcscmp(m_pData->m_String, lpsz);
    return tmp >= 1 ? 1 : (tmp <= -1 ? -1 : 0);
}
...
{
    if (pCharMap == NULL) {
        pCharMap = CFX_CharMap::GetDefaultMapper(936);
    }
    if (pCharMap == NULL) {
...
CFX_WideString CFX_WideString::FromUTF16BE(const unsigned short* wstr, FX_STRSIZE wlen)
{
    if (!wstr || 0 == wlen) {
        return CFX_WideString();
    }
    ...
    FX_WCHAR* buf = result.GetBuffer(wlen);
    for (int i = 0; i < wlen; i ++) {
        FX_WCHAR tmp = ((wstr[i] & 0xff00) >> 8) + ((wstr[i] & 0x00ff) << 8);
        buf[i] = tmp;
    }
    result.ReleaseBuffer(wlen);
    return result;
...
void CFX_WideString::FormatV(FX_LPCWSTR lpszFormat, va_list argList)
{
    ...
    GetBuffer(nMaxLen);
    if (m_pData) {
        FXSYS_memset8(m_pData->m_String, 0, (nMaxLen + 1) * sizeof(wchar_t));
