...
void CFX_MapPtrToPtr::GetNextAssoc(FX_POSITION& rNextPosition, void*& rKey, void*& rValue) const
{
    ...
    CAssoc* pAssocRet = GetCurrentAssoc(rNextPosition);
    rKey = pAssocRet->key;
    rValue = pAssocRet->value;
}
CFX_MapPtrToPtr::CAssoc	*
CFX_MapPtrToPtr::GetCurrentAssoc(FX_POSITION& rNextPosition) const
{
    ASSERT(m_pHashTable != NULL);
...
FX_BOOL CFX_MapPtrToPtr::ExpandHashTable()
{
    const FX_DWORD MaxHashTableSize = 10000;
    ...
    if (m_nCount >= static_cast<int>(m_nHashTableSize)) {
        ...
        while (nPos) {
            pOldAssoc = GetCurrentAssoc(nPos);
            nTmpHash = HashKey(pOldAssoc->key) % (m_nHashTableSize * 2);
            pOldAssoc->pNext = pTmpHashTable[nTmpHash];
            pTmpHashTable[nTmpHash] = pOldAssoc;
        }
        FX_Free(m_pHashTable);
        m_pHashTable = pTmpHashTable;
        m_nHashTableSize *= 2;
        return TRUE;
    }
    return FALSE;
...
CFX_MapByteStringToPtr::GetCurrentAssoc(FX_POSITION& rNextPosition) const
{
    ASSERT(m_pHashTable != NULL);
    CAssoc* pAssocRet = (CAssoc*)rNextPosition;
    ASSERT(pAssocRet != NULL);
...
FX_BOOL CFX_MapByteStringToPtr::ExpendHashTable()
{
    const FX_DWORD MaxHashTableSize = 10000;
    if (m_nHashTableSize > MaxHashTableSize) {
        return FALSE;
