...
#include <locale.h>
static int _Buffer_itoa(char* buf, FX_INT32 i, FX_DWORD flags)
{
    if (i == 0) {
        buf[0] = '0';
        return 1;
...
CFX_ByteString CFX_ByteString::FormatInteger(FX_INT32 i, FX_DWORD flags)
{
    char buf[32];
...
void CFX_ByteString::FormatV(FX_LPCSTR lpszFormat, va_list argList)
{
            ...
            switch (*lpsz) {
                ...
                case 'f':
                    if (nWidth + nPrecision > 100) {
                        ...
                        char pszTemp[256];
                        FXSYS_memset32(pszTemp, 0, sizeof(pszTemp));
    ...
    GetBuffer(nMaxLen);
    if (m_pData) {
        FXSYS_memset32(m_pData->m_String, 0, nMaxLen);
        CFX_ByteString saved_locale = setlocale(LC_NUMERIC, NULL);
