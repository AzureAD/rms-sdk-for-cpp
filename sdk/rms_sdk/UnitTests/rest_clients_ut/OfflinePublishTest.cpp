...
void OfflinePublishTest::testCreatePLWithTemplate()
{
    ...
    Q_ASSERT(result.name == em);
    Q_ASSERT(result.id == "00000");
}

void OfflinePublishTest::testEncryptBytes()
{
    auto pPublishClient = dynamic_pointer_cast<PublishClient>(IPublishClient::Create());

    //must call publish to generate rsa and whatnot
    PublishUsingTemplateRequest req;
    req.bAllowAuditedExtraction = false;
    req.bPreferDeprecatedAlgorithms = false;
    req.templateId = "00000";
    auto em = "john.smith@microsoft.com";
    auto d = DummyAuthCallback();
    pPublishClient->LocalPublishUsingTemplate(req, d, em, nullptr, OfflinePublishTest::generateMockCLC);

    auto val_checker = [](vector<uint8_t> bytes){
        auto first = bytes[0];
        for (auto o : bytes)
            if (o != first)
                return false;
    ...
    auto rsa = RSAKB(*pPublishClient);
    auto key = vector<uint8_t>(16, 1);
    auto bytes = EncryptBytesToBase64Wrapper(*pPublishClient, vector<uint8_t>(4096, 4), key, CipherMode::CIPHER_MODE_CBC4K);
    bytes = rsa->PrivateDecrypt(common::ConvertBase64ToBytes(bytes));
    Q_ASSERT(val_checker(bytes));
