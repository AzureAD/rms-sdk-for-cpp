...
void PlatformJsonObjectTest::testGetNamedString(bool enabled)
{
  ...
  try {
    ...
    StaticLogger::Debug("Convertion exception: %s", e.what());
    value = STR_EXCEPTION;
  }
...
void PlatformJsonObjectTest::testGetNamedObject(bool enabled)
{
  ...
  if (exsists)
  {
    try
    {
      StaticLogger::Debug("val: %s", p_namedObj->GetNamedString("str-value").data());
      StaticLogger::Debug("val: %i", p_namedObj->GetNamedBool("bool-value"));
    }
    catch (exceptions::RMSInvalidArgumentException& e)
    {
