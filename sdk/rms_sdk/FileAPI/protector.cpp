...
std::unique_ptr<Protector> Protector::Create(
  ...
  switch (pType) {
    case ProtectorType::OPC: {
      std::unique_ptr<Protector> protector(new MetroOfficeProtector(fileName, inputStream));
      return protector;
    }
    ...
    case ProtectorType::PSTAR: {
      std::unique_ptr<Protector> protector(
            new PFileProtector(protectorSelector.GetCurrentFileExtension(), inputStream));
      return protector;
    }
    default: {
