...
pplx::task<void> http::HttpClientQt::HttpGetAsync(std::string uri, common::ByteArray &res,web::http::http_headers headers, http::StatusCode &code )
{
...
void HttpClientQt::AddHeader(const string& headerName,
    ...
    web::http::http_headers::key_type i=utility::conversions::to_string_t(headerName);
...
const string HttpClientQt::GetResponseHeader(const string& headerName) {
    web::http::http_headers::key_type key= utility::conversions::to_string_t(headerName);
  return replyheaders_.has(key) ? utility::conversions::to_utf8string(replyheaders_[key]) : "";
}
