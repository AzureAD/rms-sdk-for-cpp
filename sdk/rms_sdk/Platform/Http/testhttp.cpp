...
void HttpClientQt::AddHeader(const string& headerName,
  ...
  this->req_.headers().add()(headerName.c_str(), headerValue.c_str());
}
                              ...
                              std::shared_ptr<std::atomic<bool> >cancelState)
{
  ...
  web::uri Uri=common::IUri::Create(uri);
  this->req_.set_request_uri(Uri);
  this->AddAcceptMediaTypeHeader(mediaType);

  Logger::Hidden("==> Request Headers:");
  foreach(const QByteArray &hdrName, this->req_.headers()) {
