...
bool ConsentDBHelper::Initialize()
{
    ...
    if (m_init != true)
    {
        ...
        InitializeDb(m_serviceUrlFile, path + m_serviceUrlDBFileName, m_serviceUrlCache);
        InitializeDb(m_docTrackingFile, path + m_documentTrackingDBFileName, m_serviceUrlCache);
        m_init = true;
    }
    return m_init;
}

bool ConsentDBHelper::InitializeDb(fstream& stream, const string& fullPath, vector<UserDomain>& content)
{
    stream.open(fullPath, fstream::in | fstream::out);

    if (stream.is_open())
    {
      // File exists for read, attempt to read state.
      ReadFileContent(stream, content);
      stream.close();
    }
    stream.open(fullPath, fstream::out | ios_base::app);
    return true;
}
...
bool ConsentDBHelper::ReadFileContent(fstream& stream, vector<UserDomain>& content)
{
    std::string line;
...
bool ConsentDBHelper::IsApprovedServiceDomainPresent(const string& userId, const string& domain)
{
...
bool ConsentDBHelper::IsDocumentTrackingConsentnPresent(const string& userId, const string& domain)
{
    return IsConsentPresentCommon(userId, domain, m_docTrackingCache);
}

// Adds the given entry to service url db
void ConsentDBHelper::AddApprovedServiceDomain(const string& userId, const string& domain)
{
    m_serviceUrlCache.push_back(AddConsentCommon(userId, domain, m_serviceUrlFile));
}

// Adds the given entry to doc tracking db
void ConsentDBHelper::AddDocumentTrackingConsent(const string& userId, const string& domain)
{
    m_docTrackingCache.push_back(AddConsentCommon(userId, domain, m_docTrackingFile));
}
...
// Returns all matches
vector<UserDomain>ConsentDBHelper::GetApprovedDomainsForConsent(const string&       userId,
                                                                const string&       domain,
