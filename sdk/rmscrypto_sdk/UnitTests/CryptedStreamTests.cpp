...
void CryptedStreamTests::CryptedStreamToMemory_data() {
    ...
    QTest::newRow("AES_CBC4K") << "7bddca3c7ff9bdc8eea7a8eb63d01f11" << "Plain Text" << "00000000000000000000000000000000"
                               << "ea21b48f530be23b6d654db9302dd1ff" << static_cast<qint8>(rmscrypto::api::CIPHER_MODE_CBC4K);
}

void CryptedStreamTests::CryptedStreamToMemory() {
    shared_ptr<stringstream> backingBuffer = make_shared<stringstream>(ios::in | ios::out | ios::binary);
    ...
    rmscrypto::api::CipherMode algo = static_cast<rmscrypto::api::CipherMode>(algorithm);

    memcpy(key.data(), qKey.data(), key.size());

    auto backingStream = rmscrypto::api::CreateStreamFromStdStream(static_pointer_cast<iostream>(backingBuffer));

    auto cryptoStream = rmscrypto::api::CreateCryptoStream(algo, key, backingStream);

    cryptoStream->Write(reinterpret_cast<const uint8_t*>(qData.data()), qData.size());
    cryptoStream->Flush();

    auto res = backingBuffer->str();
    QVERIFY2(static_cast<int>(res.length()) == qEncryptedData.size(), "Invalid encrypted size!");
    QVERIFY2(memcmp(res.data(), qEncryptedData.data(), res.length()) == 0,
             "Invalid encrypted data!");

    cryptoStream->Seek(0);
    QByteArray dst(qData.size(), 0);
    auto read = cryptoStream->Read(reinterpret_cast<uint8_t*>(dst.data()), dst.size());
