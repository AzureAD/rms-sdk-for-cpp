...
FileCache::FileCache(const String& filePath)
{
    if (filePath.empty()) // using default settings
    {
        ...
        auto path = QStandardPaths::writableLocation(QStandardPaths::HomeLocation)+"/.ms-ad";
        if(!QDir().mkpath(path))
        {
    ...
    else
    {
        ...
        {
            throw RmsauthException("Can't find user defined cache directory", userDefinedCacheDir.toStdString());
        }
        cacheFilePath_ = fi.absoluteFilePath().toStdString();
    }
